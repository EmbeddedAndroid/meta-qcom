...
- `Signed-off-by`, `Acked-by`, `Reviewed-by` and similar trailers for anyone
  other than you must reflect approvals that were actually given. The automated
  check cannot prove this and only flags such trailers for a maintainer to
  confirm; do not fabricate a review or sign-off trail.
...
- Prefix scoped changes with `component: ` (with a space after the colon).
...
### Automated commit policy check

The deterministic subset of these rules is enforced on every pull request by the
[commit-policy-action](https://github.com/EmbeddedAndroid/commit-policy-action)
reusable workflow (`.github/workflows/commit-policy.yml`), which posts a review
and requests changes when a commit violates the policy. To run the same check
locally before opening a PR:

```sh
git clone https://github.com/EmbeddedAndroid/commit-policy-action
python3 commit-policy-action/commit_policy_check.py --base origin/master --head HEAD
```

Semantic expectations, such as whether the body explains why, whether the
subject truthfully describes the diff, and whether unrelated changes are
bundled, are not machine-checked and remain part of human review.
